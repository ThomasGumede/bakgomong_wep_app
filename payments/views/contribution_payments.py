...
from payments.models import KgotlaBalance
...
def generate_payment_link(payment: Payment):
    """Generate Yoco payment link for a given Payment record."""
    try:
        str_amount = decimal_to_str(payment.amount)
        contr = payment.member_contribution
        session_data = {
            'successUrl': reverse("payments:payment-success", kwargs={"id": contr.id}),
            'cancelUrl': reverse("payments:payment-cancelled", kwargs={"id": contr.id}),
            "failureUrl": reverse("payments:payment-failed", kwargs={"id": contr.id}),
        ...
        return response_data["redirectUrl"]

    except Exception as err:
        logger.error(f"Error generating Yoco payment link: {err}")
        raise
